...
class snake:
    ...
    def man_dist(self, point1, point2):
        #manhattan distance heuristic between point1 and point2 which are tuples as (x,y)
    ...
    def astar(self,start,goal,tail_chasing = False):
        ...
        self.f_score = {(x, y): float('inf') for x in range(int(grid.x)) for y in range(int(grid.y))}
...
pygame.display.set_caption("Aracı Yılan Oyunu")
