...
class fruit:
    ...
    def move(self, blockade = []):#used when the food is eaten
        
        available = {(x * grid_size, y * grid_size) for x in range(int(grid.x)) for y in range(int(grid.y))}
        blockade = {(x, y) for x, y in blockade}
        available = available - blockade
        if not available:
            print("AI WINS!")
            return True
        
        available = random.choice(  list(available) )
        
        self.pos = Vector2(available)
...
class snake:
    ...
    def __init__(self,map_size):
        ...
        self.head_color = (127,127,200)
        self.body_color = (127,127,0)
    ...
    def move(self):
        ...
        if self.path:
            self.direction = - (Vector2(self.body[0]) - self.path.pop(0) ) / grid_size   
    ...
    def man_dist(self, point1, point2):
        ...
        return abs(point1[0] - point2[0]) / grid_size + abs(point1[1] - point2[1]) / grid_size
    ...
    def can_reach_tail(self, planned_path, tail_chasing = False):
        ...
        full_body = simulated_body[:-1]
            
    
        snake_body_set = {(int(segment.x // grid_size), int(segment.y // grid_size)) for segment in full_body}
        
        if self.bfs_reach_tail(start, tail, snake_body_set):
            self.body = original_body
            self.direction = original_direction
            return True
        
        # Step 5: Restore original state as a fail-safe
        self.body = original_body
        self.direction = original_direction
    ...
    def tail_chasing_mode(self):
        ...
        directions = [(-grid_size, 0), (0, grid_size),  (0, -grid_size), (grid_size, 0)]
        
        for path in directions:
            part = Vector2(path) + head
            self.planned_path = [part]
            if tail == part:
                tmp = path
                
                continue
            if self.can_reach_tail(self.planned_path, True):
                if (0 <= part[0] < size.x and 
                    0 <= part[1] < size.y and 
                    not self.is_obstacle(part, self.body[:])):
                    
                    self.direction = Vector2(path) / grid_size
                    return
        
        self.direction = Vector2(tmp) / grid_size
        self.planned_path = [tail]
    ...
    def astar(self,start,goal,tail_chasing = False):
        ...
        snake_body_set = {(int(segment.x // grid_size), int(segment.y // grid_size)) for segment in self.body}
        
        if not self.bfs_reach_tail(    tuple(start / grid_size)
                               , tuple(goal / grid_size)
                               , snake_body_set):
            #if tail won't be reach, don't try the A* algorithm
        ...
        self.f_score = {(grid_size * x, grid_size * y): float('inf') for x in range(int(grid.x)) for y in range(int(grid.y))}
        ...
        while open_set:
                    ...
                    if not self.can_reach_tail(self.planned_path):   
                        ...
                        for path in [(0, grid_size), (grid_size, 0), (0, -grid_size), (-grid_size, 0)]:
                            ...
                            and self.f_score[space] < float('inf') and space in came_from):
                                ...
                                break
                        continue
                    self.path = self.planned_path
                    self.direction = - (Vector2(start) - Vector2(path[-2]) ) / grid_size                    
                return
            ...
            for dx, dy in [(0, grid_size), (grid_size, 0), (0, -grid_size), (-grid_size, 0)]:
                ...
                if (0 <= neighbor[0] < size.x and 
                    ...
                    not self.is_obstacle(neighbor, self.body[1:])):
    ...
    def is_obstacle(self, position, obstacles):
        ...
        #check collision with snake body (except the head, and tail when not growing)
        if not self.grow:
            obstacles.pop()
...
pygame.display.set_caption("Final Aracı Yılan Oyunu")
...
while running:
    ...
    if paused:
        ...
        pygame.draw.rect(screen
                         , Vector3(snAIke.head_color) - Vector3(snAIke.body_color)
                         , head_rect)
        segments = []
        Snek_len = len(snAIke.body)
        for seg_num in range(   len(snAIke.body[1:])   ):
            segment = snAIke.body[seg_num + 1]
            segment_rect = pygame.Rect(segment.x, segment.y, snAIke.size, snAIke.size)
            pygame.draw.rect(screen
                             , Vector3(snAIke.head_color) -  Vector3(snAIke.body_color) * (Snek_len - seg_num) / Snek_len  
                             , segment_rect)
    ...
    if fruit.colliderect(head_rect):
        prev = fruit
        if main_fruit.move(snAIke.body):
            running = False
            break
...
print(f"Game over. Score: {score}, Game Cycles: {counter}")
pygame.quit()